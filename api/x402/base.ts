...
const routes: RoutesConfig = {
  [`GET ${route}`]: {
    ...
    serviceName: "AgentPay Firewall Base compatibility resource",
    unpaidResponseBody: () => ({
      contentType: "application/json",
      body: {
        error: "Payment required",
        integration: "official-x402-base",
        message:
          "This compatibility challenge is generated by @x402/express for Base Sepolia.",
        network: config.network,
...
app.get(route, (_request, response) => {
  response.json({
    ...
    integration: {
      protocol: "x402",
      implementation: "@x402/express + @x402/evm",
      network: config.network,
      facilitatorUrl: config.facilitatorUrl,
      payTo: config.payTo,
      price: config.price,
